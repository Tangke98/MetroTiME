#' Annotates cell metabolism for the Metacells of specific cell lineages.
...
saveRDS(inte_use,paste0(output_path,'_integration_metabolic_cluster.rds'))
...
output_name='_metatype_cluster_original_marker.pdf'
width=20
height=4
...
cal_MetaModule(metacell,MM,output_path,'_gsva_nodup')

## read the metadata
MM.meta.nozero=readRDS(MetaModule_info) %>% as.data.frame()

## enlarge our results
gsva_score=readRDS(paste0(output_path,'_gsva_nodup.rds'))
new_rows <- list()
...
saveRDS(gsva_score_enlarge,paste0(output_path,'_gsva.rds'))

## use the MetaModule scores to annotate cell metabolic states
gsva.seurat <- CreateSeuratObject(counts = gsva_score, project = cell_lineage, min.cells = 0, min.features = 0)
...
saveRDS(MetaModule.markers,paste0(output_path,'_metatype_cluster_DEM.rds'))
...
saveRDS(inte_use,paste0(output_path,'_integration_annotation_celltype_metatype.rds'))
saveRDS(inte_use@meta.data,paste0(output_path,'_integration_annotation_celltype_metatype_metadata.rds'))
...
output_name='_metatype_marker.pdf'
width=6
height=4
...
inte_use<-readRDS(paste0(output_path,'_integration_annotation_celltype_metatype.rds'))
gsva_score_enlarge<-readRDS(paste0(output_path,'_gsva.rds'))

MM.meta.nozero=readRDS(MetaModule_info) %>% as.data.frame()

## use the MetaModule scores to annotate cell metabolic states
gsva.seurat <- CreateSeuratObject(counts = gsva_score_enlarge, project = cell_lineage, min.cells = 0, min.features = 0)
...
saveRDS(MetaModule.markers,paste0(output_path,'_metatype_type_DEM.rds'))
...
saveRDS(markers,paste0(output_path,'_metatype_type_DEG.rds'))