#' Uses Clustree to evaluate the clustering of myeloid cells based on metabolic genes.
...
multi_res=function (resolution, inte_data) 
{
    for (res in resolution) { ## by the silhouette score, we have idenified the best dims and k.param to cluster the metacells
        ...
        inte_data <- FindNeighbors(inte_data, dims = 1:45, k.param = 80)
...
output_name='myeloid_metabolic_pc45_cutree'
width=8
height=6
