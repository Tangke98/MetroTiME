...
Seurat_TPM_fibroblasts<- args[1]
Seurat_TPM_metadata_fibroblasts<- args[2]
TF_Ligand_Metabolic<- args[3] ## integrated information
integrated_object_path_fibroblasts<-args[4]

data<-readRDS(Seurat_TPM_fibroblasts)
data <- CreateSeuratObject(counts = data, project = "fibroblasts", min.cells = 0, min.features = 0)
metadata<-readRDS(Seurat_TPM_metadata_fibroblasts)
...
saveRDS(anchors,paste0(integrated_object_path_fibroblasts,'fibroblasts_anchors.rds'))
RNA.integrated <- IntegrateData(anchorset = anchors, dims = dims.use)
saveRDS(RNA.integrated,paste0(integrated_object_path_fibroblasts,'fibroblasts_integration.rds'))