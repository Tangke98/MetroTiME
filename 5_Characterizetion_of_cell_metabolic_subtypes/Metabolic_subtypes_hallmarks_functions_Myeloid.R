#' Corresponds metabolic subtypes and cell functions in myeloid cells.
...
cal_MetaModule=function (object, module_list, output_path, file_name) 
{
    if (ncol(object) > 10000) {
        ...
        if (length(module_list) > 5) {
            group_indices <- cut(seq_along(module_list), breaks = seq(1, 
                length(module_list), by = 5), labels = FALSE)
            ...
            for (i in 1:length(sublists)) {
                ...
                gsva <- gsva(object, MM_use, method = "gsva", 
                  min.sz = 1, max.sz = 500, kcdf = "Gaussian", 
                  mx.diff = TRUE, verbose = TRUE, parallel.sz = 5)
                saveRDS(gsva, paste0(output_path, "MM_tmp/", 
                  output_name_use, ".rds"))
            }
            files = list.files(paste0(output_path, "MM_tmp/"))
            merge = function(x) {
                data <- readRDS(paste0(paste0(output_path, "MM_tmp/"), 
                  x))
                return(data)
            }
            merge_res = do.call(rbind, lapply(as.list(files), 
                merge))
            saveRDS(merge_res, paste0(output_path, file_name, 
                ".rds"))
        ...
        if (length(module_list) > 100) {
            group_indices <- cut(seq_along(module_list), breaks = seq(1, 
                length(module_list), by = 100), labels = FALSE)
            sublists <- split(module_list, group_indices)
            for (i in 1:length(sublists)) {
                MM_use = sublists[[i]]
                output_name_use = paste0(file_name, "_", names(sublists[i]))
                gsva <- gsva(object, MM_use, method = "gsva", 
                  min.sz = 1, max.sz = 500, kcdf = "Gaussian", 
                  mx.diff = TRUE, verbose = TRUE, parallel.sz = 50)
                saveRDS(gsva, paste0(output_path, "MM_tmp/", 
                  output_name_use, ".rds"))
            }
            files = list.files(paste0(output_path, "MM_tmp/"))
        ...
        gsva <- gsva(object, module_list, method = "gsva", min.sz = 1, 
            max.sz = 500, kcdf = "Gaussian", mx.diff = TRUE, 
...
classical_mono=c('CCR2','CD14','VCAN','S100A8','S100A9','FCN1','CLEC4D','CLEC5A',
                     'CXCR1','CXCR2','IL13RA1')
non_classical_monoc=c('CX3CR1','CSFR1','NR4A1','BCL2A1A','DUSP5','HES1','POU2F2','TCF7L2',
                         'CX3CR1','SPN','ITGAL','ITGAX','PECAM1','RRAS',
                         'FCGR3','TNF', 'IL10R','IL1B','IL6','IL1RA', 'TGFBR3', 'LAIR1', 'LTB', 'KLRD1', 'GZMA',
                         'TLR7', 'IFITM1', 'IFITM2', 'IFITM3')
intermediate_mono=c('NAMPT','NEAT1','G0S2','CSF3R','CCR5','CD32','CD40','CD80','GFRA2','TNFR1')
...
names(features)=c('Anti_inflammatory','Pro_inflammatory','M1','M2','Antigen_Presenting','Angiogenesis',
                  'Phagocytosis','ECM', 'MDSC','cell_cycle','classical_mono','non_classical_monoc','intermediate_mono')


DefaultAssay(myeloid)='integrated'
metacell=as.matrix(GetAssayData(myeloid))
cal_MetaModule(metacell,features,output_path,'myeloid_subset_features_gsva')

### calculate hallmarks scores for each metabolic states
hallmarks=readRDS(hallmark_path)
cal_MetaModule(metacell,hallmarks,output_path,'myeloid_subset_hallmarks_gsva')

## visualization of the feature scores and hallmarks scores
### for feature scores
output_name='myeloid_feature_ssgsea_res.pdf'
width=3
height=5

feature_score<-readRDS(paste0(output_path,'myeloid_subset_features_gsva.rds'))
# myeloid@meta.data=myeloid@meta.data[,1:13]
Idents(myeloid)='metabolic_type'
...
p1 <- DotPlot(object, features = c('Anti_inflammatory','Pro_inflammatory','M1','M2','Antigen_Presenting','Angiogenesis','Phagocytosis','ECM','cell_cycle'),assay='RNA' ) + 
    coord_flip() + 
...
### for hallmarks score
output_name='myeloid_hallmark_ssgsea_res.pdf'
width=4
height=4

hallmark<-readRDS(paste0(output_path,'myeloid_subset_hallmarks_gsva.rds'))
