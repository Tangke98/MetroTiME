#' Performs MetaRegulon analysis on myeloid cells.
...
for (i in 1:nrow(module_df)){
    file_res=paste0(integrated_object_path_myeloid,module_df[i,2],'/MetaRegulon/',module_df[i,2],':',module_df[i,1],'.txt')
    if (!file.exists(file_res)){
        base=paste0(integrated_object_path_myeloid,module_df[i,2],'/MetaRegulon/')
...
data=GetAssayData(myeloid)
data_use=as.data.frame(t(as.matrix(data[res_rbind$gene,])))
data_use$metabolic_type=myeloid$metabolic_type
data_use$Cancer_type=myeloid$Cancer_type
...
pdf(paste0(output_path,"myeloid_regulon_tf_expression_metabolictype.pdf"),width=width,height=height)
    print(p) 
dev.off()